...
const errorHandler = (err, req, res, next) => {
  ...
  // Error personalizado para parámetros de geolocalización inválidos
  if (err.message.startsWith("ERROR 020")) {
    const message = err.message.split(", ")[1] || "Parámetros de geolocalización incorrectos";
    res.status(400).json({ message });
    return;
  }
