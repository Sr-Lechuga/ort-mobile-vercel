...
inscriptionSchema.post("findOneAndDelete", async function (doc) {
  try {
    await mongoose.model("ActivityInstance").findByIdAndUpdate(doc.instance, { $pull: { inscriptions: doc._id } });
    await mongoose.model("Volunteer").findByIdAndUpdate(doc.volunteer, { $pull: { inscriptions: doc._id } });
  } catch (err) {
...
inscriptionSchema.post("save", async function (doc, next) {
  try {
    await mongoose.model("ActivityInstance").findByIdAndUpdate(doc.instance, { $push: { inscriptions: doc._id } });
    await mongoose.model("Volunteer").findByIdAndUpdate(doc.volunteer, { $push: { inscriptions: doc._id } });
    next();
  } catch (err) {
